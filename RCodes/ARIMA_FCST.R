...
binary_lag_var = c("FLG_SU_EOSS","FLG_WN_EOSS","FLG_SP_EOSS","FLG_RAMADAN","FLG_EID","FLG_MID_SSN_OFFER")
continuous_lag_var = c("DISC_PER","DSP_STR")
#binary_nolag_var = c("PHS_FLG_1","PHS_FLG_2","PHS_FLG_3","PHS_FLG_4","DISCOUNT_PER")
binary_nolag_var = c("FLG_SALE","OOS_PER_A","OOS_PER_B","OOS_PER_C","FLG_NATIONAL_DAY","FLG_BTS","FLG_PROMO_CATG1","FLG_PROMO_CATG2","FLG_PROMO_CATG3","FLG_PROMO_CATG4")
...
windows.hist_TS = "C:/Users/abhishek.meena/Desktop/IM/MAX_LDS_R_PROJECT/DATA/SD_MAX_LDS_FSCT_HADS_TS2.txt"
windows.lead_TS = "C:/Users/abhishek.meena/Desktop/IM/MAX_LDS_R_PROJECT/DATA/SD_MAX_LDS_FSCT_LADS_TS2.txt"
...
#input.DF.forecast <- input.DF.forecast[which(input.DF.forecast$KEY=="Children|Baby boys newborn|Bodysuits|Bodysuits|LBB891NOOS|WHITE|LIGHT"),]
#input.DF.lead <- input.DF.lead[which(input.DF.lead$KEY=="Children|Baby boys newborn|Bodysuits|Bodysuits|LBB891NOOS|WHITE|LIGHT"),]

#input.DF.forecast <- input.DF.forecast[which(input.DF.forecast$STND_TRRTRY_NM=="Bahrain"),]
#input.DF.lead <- input.DF.lead[which(input.DF.lead$STND_TRRTRY_NM=="Bahrain"),]
...
AR01.forecast <- function(AR01.train.X,AR01.train.XREG,model.type) {
  ...
  arima.fit.tryCatch <- function(error.ind) {
    if (error.ind == 'g') {
      model.fit <- auto.arima(AR01.train.X,xreg=train.XREG_zerovar)
    }
    else if (error.ind == 'w') {
      model.fit <- auto.arima(AR01.train.X,xreg=train.XREG_zerovar)
    }
  ...
  AR01.model.auto <- tryCatch({
    ...
    print(paste("auto.arima warning: ",w))
    ind = 'w'
    fit.tryCatch <- arima.fit.tryCatch(ind)
    return(fit.tryCatch)
...
AR01.lead.forecast <- function(AR01.model,lead.XREG,model.type,num.zerovar,xreg.zerovar) {
  ...
  if (model.type == "auto") {
    AR01.lead.fcst <- tryCatch({
      ...
      print(paste("auto.arima error: ",war))
      ind = 'w'
      predict.tryCatch <- arima.predict.tryCatch(ind)
...
ets.forecast <- function(ets.hist){
  
  ets.model <- ets(ets.hist 
                   ,model="ZNZ"
                   ,lower=c(0.0001,0.0001,0.0001,0.8)
...
AR.parm.simulation <- function(AR01.parm.ts,AR01.ts.exgvar) {
  ...
  if (nrow(AR01.ts.exgvar) >= 78) {
    ...
    clusVal <- sample(52:(length(AR01.parm.ts)-cv.fold),cv.iteration,replace=F)
  }
...
AR01.forecast.engine <- function(ts.DF,lead.DF,lead,seedval) {
  ...
  for(t in 1:length(AR01.territory)) {
    ...
    hist.TERR.DF = ts.DF[which(ts.DF$STND_TRRTRY_NM==as.character(AR01.territory[t])),]
    
    AR01.clss_nm <- unique(hist.TERR.DF$KEY)
    
    for(c in 1:length(AR01.clss_nm)) {
      
      print(paste(as.character(AR01.territory[t])," - ",as.character(AR01.clss_nm[c]),sep=""))
      
      hist_info_ts = hist.TERR.DF[which(hist.TERR.DF$STND_TRRTRY_NM==as.character(AR01.territory[t]) & 
                                          
                                          hist.TERR.DF$KEY==as.character(AR01.clss_nm[c])),]
      ...
      lead_info_ts = lead.DF[which(lead.DF$STND_TRRTRY_NM==as.character(AR01.territory[t]) & 
                                     ...
                                     lead.DF$KEY==as.character(AR01.clss_nm[c])),]
      ...
      if (nrow(hist_exgvar) >= 52) {
        ...
        hist_info_ts$ARIMA_FCST <- c(rep(NA,(nrow(hist_info_ts)-length(AR01.hist.fcst)))
                                     ,AR01.hist.fcst)
        
        lead_info_ts <- lead_info_ts[1:lead,c(1:hrchy_column)]
        #lead_info_ts$OLD_HRCHY = with(lead_info_ts, paste(MAPPED_ATTR_LVL_1,"|",MAPPED_ATTR_LVL_2,"|",MAPPED_ATTR_LVL_3,"|",MAPPED_ATTR_LVL_4,"|",MAPPED_ATTR_LVL_5))
        
        lead_info_ts <- cbind(lead_info_ts,lead.exgvar)
        lead_info_ts$TIME_FRAME <- c(rep("Lead Period",nrow(lead_info_ts)))
        
        lead_info_ts$ARIMA_FCST <- AR01.lead.fcst
...
es.forecast.engine<-function(ts.DF,lead.DF) {
  ...
  ETS.territory <- unique(ts.DF$STND_TRRTRY_NM)
  
  ETS_ATTR_FCST_FINAL <- data.frame()
  ETS_ATTR_FCST_PARM <- data.frame()
  
  for(t in 1:length(ETS.territory)) {
    
    hist.TERR.DF = ts.DF[which(ts.DF$STND_TRRTRY_NM==as.character(ETS.territory[t])),]
    
    ES01.clss_nm <- unique(hist.TERR.DF$KEY)
    
    for(c in 1:length(ES01.clss_nm)) {
      
      print(paste(as.character(ETS.territory[t])," - ",as.character(ES01.clss_nm[c]),sep=""))
      
      hist_info_ts = hist.TERR.DF[which(hist.TERR.DF$STND_TRRTRY_NM==as.character(ETS.territory[t]) & 
                                          
                                          hist.TERR.DF$KEY==as.character(ES01.clss_nm[c])),]
      
      lead_info_ts = lead.DF[which(lead.DF$STND_TRRTRY_NM==as.character(ETS.territory[t]) & 
                                     
                                     lead.DF$KEY==as.character(ES01.clss_nm[c])),]
      
      hist_info_ts$ETS_RTL_QTY = ifelse(hist_info_ts$ETS_RTL_QTY <= 0, .001, hist_info_ts$ETS_RTL_QTY)
      
      hist_info_ts$TRDNG_WK_STRT_DT <- as.Date(hist_info_ts$TRDNG_WK_STRT_DT,format="%d%b%Y")
      ...
      if(nrow(hist_info_ts) >= 13) {
        
        TS.start.dt <- min(hist_info_ts$TRDNG_WK_STRT_DT)
        
        hist.ts.format <- ts(hist_info_ts$ETS_RTL_QTY, freq=52, start=decimal_date(ymd(TS.start.dt)))
        
        ETS.forecast.obj = ets.forecast(hist.ts.format)
        
        ets.fitted.val = data.frame(as.numeric(ETS.forecast.obj$fitted))
        names(ets.fitted.val)[1] <- "ETS_FCST"
        
        hist_info_ts$TIME_FRAME <- "Hist Period"
        hist_info_ts <- data.frame(cbind(hist_info_ts,ets.fitted.val))
        
        ets.lead.fcst = data.frame(as.numeric(forecast(ETS.forecast.obj,h=nrow(lead_info_ts),method ='ets')$mean))
        names(ets.lead.fcst)[1] <- "ETS_FCST"
        
        lead_info_ts$TIME_FRAME <- "Lead Period"
        lead_info_ts <- data.frame(cbind(lead_info_ts,ets.lead.fcst))
        
        ETS_forecast_ts <- rbind.fill(hist_info_ts,lead_info_ts)
        ETS_forecast_ts$ETS_FLG <- c(rep(1,nrow(ETS_forecast_ts)))
        
        ets.parm <- data.frame(ETS.forecast.obj$par)
        parm.name <- rownames(ets.parm)
        ets.parm <- cbind(parm.name,value=ets.parm)
        
        ets_forecast_parm <- cbind(hist_info_ts[1,c(1:join_column)],method=ETS.forecast.obj$method,ets.parm)
        rownames(ets_forecast_parm) <- NULL
      }
      else {
        
        hist_info_ts$TIME_FRAME <- c(rep("Hist Period",nrow(hist_info_ts)))
        hist_info_ts$ETS_FCST <- NULL
        
        lead_info_ts <- lead_info_ts[1:lead,]
        lead_info_ts$TIME_FRAME <- c(rep("Lead Period",nrow(lead_info_ts)))
        lead_info_ts$ETS_FCST <- NULL
        
        #lead_info_ts$WK_STRT_D <- with(lead_info_ts,as.Date(as.character(WK_STRT_D),format="%d%b%Y"))
        #lead_info_ts$WK_END_D <- with(lead_info_ts,as.Date(as.character(WK_END_D),format="%d%b%Y"))
        
        ETS_forecast_ts <- rbind.fill(hist_info_ts,lead_info_ts)
        ETS_forecast_ts$ETS_FLG <- c(rep(0,nrow(ETS_forecast_ts)))
        
        ets_forecast_parm <- hist_info_ts[1,c(1:join_column)]
        #AR01.model.coef <- hist_info_ts[1,c(1:join_column)]
      }
      
      ETS_ATTR_FCST_FINAL <- rbind.fill(ETS_ATTR_FCST_FINAL,ETS_forecast_ts)
      ETS_ATTR_FCST_PARM <- rbind.fill(ETS_ATTR_FCST_PARM,ets_forecast_parm)
      #ETS.intime_mape <- ets.validation(hist_info_ts,ETS_forecast_hist)
    }
  }
  return(list(ETS_ATTR_FCST_FINAL,ETS_ATTR_FCST_PARM))
}
...
op.ETS.Forecast <- es.forecast.engine(input.DF.forecast,input.DF.lead)
...
ETS_ATTR_FCST_FINAL <- op.ETS.Forecast[[1]]
ETS_ATTR_FCST_PARM <- op.ETS.Forecast[[2]]

AR01_ATTR_FCST_FINAL$AR_ORDR_FCST <- ifelse(AR01_ATTR_FCST_FINAL$AR_ORDR_FCST <= 0, 0, AR01_ATTR_FCST_FINAL$AR_ORDR_FCST)
ETS_ATTR_FCST_FINAL$ETS_FCST <- ifelse(ETS_ATTR_FCST_FINAL$ETS_FCST <= 0, 0, ETS_ATTR_FCST_FINAL$ETS_FCST)
...
COMB_OPTION_FCST_FINAL <-sqldf("select 
                               a.*
                               , b.ETS_FCST
                               , b.ETS_FLG
                               from AR01_ATTR_FCST_FINAL a 
                               left join ETS_ATTR_FCST_FINAL b 
                               on a.STND_TRRTRY_NM = b.STND_TRRTRY_NM
                               and a.KEY = b.KEY
                               and a.TRDNG_WK_END_DT = b.TRDNG_WK_END_DT"
)
...
write.table(COMB_OPTION_FCST_FINAL, "D:/INTEGRATED FORECASTING/OPTION/MAX/R/Export/SD_MAX_CLN_FSCT_ROUT_v2.txt", sep=",", row.names=FALSE)
write.table(COMB_OPTION_FCST_FINAL, "D:/INTEGRATED FORECASTING/OPTION/MAX/SAS/Import/SD_MAX_CLN_FSCT_ROUT_v2.txt", sep=",", row.names=FALSE)
